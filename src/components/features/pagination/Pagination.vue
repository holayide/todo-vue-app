...
const page = computed(() => props.data?.meta?.page ?? 1);
...
const paginationRange = computed(() =>
  createPaginationRange(page.value, totalPages.value)
);
</script>
    ...
    <div class="flex gap-2 flex-wrap">
      <template v-for="(pageNum, idx) in paginationRange" :key="idx">
        ...
        <button
          ...
          :class="{
            'bg-blue-600 text-white cursor-not-allowed': pageNum === page,
            'bg-input-bg hover:bg-pag-hover cursor-pointer': pageNum !== page,
          }"
        >
          {{ pageNum }}
        </button>
